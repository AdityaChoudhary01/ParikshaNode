...
import { Clock, HelpCircle, Tag, BookOpen, LayoutDashboard, BarChart, Trophy, FileText } from 'lucide-react';
import { Helmet } from 'react-helmet-async';

const HomePage = () => {
  const { user } = useSelector((state) => state.auth);
  const { data: quizzes, isLoading: quizzesLoading, error } = useFetch('/quizzes');
      ...
      <div className="space-y-20">
        ...
        <section className="text-center py-20">
          ...
          <div className="mt-8 flex justify-center gap-4">
            ...
            {user ? (
              user.role === 'admin' ? (
                <Link to="/admin"><Button size="lg" variant="outline">Admin Dashboard</Button></Link>
              ) : (
                <Link to="/history"><Button size="lg" variant="outline">My History</Button></Link>
              )
            ) : (
        ...
        <section className="text-center">
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Card><CardHeader className="items-center"><div className="p-3 bg-primary/10 rounded-full"><BookOpen className="w-8 h-8 text-primary" /></div><CardTitle>Diverse Categories</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">From programming to history, find quizzes on a wide range of subjects.</p></CardContent></Card>
            <Card><CardHeader className="items-center"><div className="p-3 bg-primary/10 rounded-full"><LayoutDashboard className="w-8 h-8 text-primary" /></div><CardTitle>Create & Share</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">Any user can create and share their own quizzes with a unique link.</p></CardContent></Card>
            <Card><CardHeader className="items-center"><div className="p-3 bg-primary/10 rounded-full"><BarChart className="w-8 h-8 text-primary" /></div><CardTitle>Instant Results</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">Get immediate feedback with detailed answer reviews and track your performance.</p></CardContent></Card>
          </div>
        </section>
                        ...
                        {hasAttempted ? (
                          <div className="w-full flex gap-2">
                            <Link to={`/results/${resultId}`} className="flex-1">
                              <Button variant="outline" className="w-full"><FileText className="w-4 h-4 mr-2"/>View Results</Button>
                            </Link>
                            <Link to={`/leaderboard/${quiz._id}`} className="flex-1">
                              <Button variant="secondary" className="w-full"><Trophy className="w-4 h-4 mr-2"/>Leaderboard</Button>
                            </Link>
                          </div>
                        ) : (
        ...
        <section className="text-center">
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Card><CardContent className="pt-6"><blockquote className="italic text-muted-foreground">"Excellent for brushing up on my data structures knowledge. The user-creation tools are fantastic!"</blockquote><p className="font-semibold mt-4">- Rohan Sharma</p><p className="text-sm text-muted-foreground">SDE at Infosys, Bengaluru</p></CardContent></Card>
            <Card><CardContent className="pt-6"><blockquote className="italic text-muted-foreground">"The clean UI and dark mode are perfect for late-night study sessions before my semester exams. A must-have tool!"</blockquote><p className="font-semibold mt-4">- Priya Singh</p><p className="text-sm text-muted-foreground">Student at IIT Delhi</p></CardContent></Card>
            <Card><CardContent className="pt-6"><blockquote className="italic text-muted-foreground">"As a UPSC aspirant, the ability to take timed quizzes on history and polity has been invaluable for my preparation."</blockquote><p className="font-semibold mt-4">- Anjali Gupta</p><p className="text-sm text-muted-foreground">UPSC Aspirant</p></CardContent></Card>
          </div>
        </section>
      </div>
    </>
